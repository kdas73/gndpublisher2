...
public class PromptLoader {
    ...
    public String load(String promptVersion) {
        if (promptVersion == null || promptVersion.isBlank()) {
            throw new LlmIntegrationException("Prompt version must not be blank");
        }
    ...
    private String loadUncached(String promptVersion) {
        ...
        if (!resource.exists()) {
            throw new LlmIntegrationException("Prompt resource not found for version: " + promptVersion);
        }

        try (var inputStream = resource.getInputStream()) {
            String prompt = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8).strip();
            if (prompt.isBlank()) {
                throw new LlmIntegrationException("Prompt resource is blank for version: " + promptVersion);
            }
            return prompt;
